...
from capsule.db.agent_memory import AgentConversationRepository, MemoryVectorOutboxEvent
...
async def test_memory_mutation_creates_and_completes_a_durable_vector_outbox_event() -> None:
    ...
    try:
        ...
        sync_state = await repository.get_sync_state(
            thread_id=thread.thread_id,
            user_id=user_id,
            workspace_id=workspace_id,
        )
        assert sync_state.last_message_sequence == 1
        assert sync_state.memory_revision == 0
        persisted_assistant = await repository.append_message(
            thread_id=thread.thread_id,
            user_id=user_id,
            workspace_id=workspace_id,
            role="assistant",
            content="好的，我会使用中文回答。",
            request_id=f"assistant-request-{suffix}",
            turn_id=f"turn-{suffix}",
        )
        assert persisted_assistant.sequence == 2
        sync_state = await repository.get_sync_state(
            thread_id=thread.thread_id,
            user_id=user_id,
            workspace_id=workspace_id,
        )
        assert sync_state.last_message_sequence == 2
        ...
        async with database.session() as session:
            outbox = await session.scalar(
                select(AgentMemoryVectorOutbox).where(
                    AgentMemoryVectorOutbox.memory_id == memory_id
                )
            )
        assert outbox is not None
        assert outbox.status == "pending"
        vector_event = MemoryVectorOutboxEvent(
            event_id=outbox.event_id,
            memory_id=outbox.memory_id,
            memory_version=outbox.memory_version,
        )
        ...
        async with database.session() as session:
            completed_outbox = await session.get(
                AgentMemoryVectorOutbox,
                vector_event.event_id,
            )
