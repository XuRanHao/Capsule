"""Reusable vector fusion for dimension-aware clustering.

The selected feature dimension can be blended with the Asset's native
multimodal embedding.  Both components are normalized before blending so the
configured weights describe semantic influence rather than vector magnitude.
"""
...
DEFAULT_NATIVE_CONTENT_WEIGHT = 0.5
...
def fuse_native_dimension_vectors(
    *,
    ...
    ``native_content_weight=1`` keeps native multimodal semantics only, while
    ``0`` keeps the selected dimension only.  A zero-weight component is not
    read or validated, allowing callers to avoid fetching it altogether.
    """
...
def _as_normalized_vector(vector: Sequence[float] | None, *, component: str) -> np.ndarray:
    if vector is None:
